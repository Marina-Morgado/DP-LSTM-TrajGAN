...
def get_trajectories(train_file, test_file, tid_col='tid',
    ...
    # logger.log(Logger.INFO, 'x_train shape: ' + str(x_train.shape)) #(num_Cols, num_rows)
    # logger.log(Logger.INFO, 'y_train shape: ' + str(y_train.shape)) #(num_Cols, num_rows)
    # logger.log(Logger.INFO, 'x_test shape:  ' + str(x_test.shape)) #(num_Cols, num_rows)
    # logger.log(Logger.INFO, 'y_test shape:  ' + str(y_test.shape)) #(num_Cols, num_rows)
...
opt = tensorflow_privacy.DPKerasSGDOptimizer(1, 0.2, 1, 0.001)
...
epsilon = compute_dp_sgd_privacy.compute_dp_sgd_privacy(44810,256,0.01,2000,1e-8)
print(epsilon)
